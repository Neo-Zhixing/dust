...
use ash::vk;
use gpu_alloc::{MemoryBlock, MemoryDevice};
...
impl UniformArray {
    ...
    unsafe fn resize_and_clear(&mut self, new_capacity: u32, device: &ash::Device, allocator: &mut crate::Allocator) {
        ...
        let staging_buf = device.create_buffer(&vk::BufferCreateInfo::builder()
        .size(array_size)
        ...
        let device_buf = device.create_buffer(&vk::BufferCreateInfo::builder()
        .size(array_size)
        ...
        // Map memory
        device.bind_buffer_memory(device_buf, *device_mem.memory(), device_mem.offset()).unwrap();
        device.bind_buffer_memory(staging_buf, *staging_mem.memory(), staging_mem.offset()).unwrap();
        self.staging_ptr = staging_mem.map(AshMemoryDevice::wrap(device), 0, array_size as usize).unwrap().as_ptr() as *mut c_void;
    ...
    pub unsafe fn write(&mut self, items: impl ExactSizeIterator<Item = UniformEntry>, device: &ash::Device, allocator: &mut crate::Allocator) {
        if items.len() == 0 {
            return;
        }
